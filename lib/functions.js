...
const wall = async(query) => { // Sobre o comando de Wallpapers
    ...
    const response = await fetch(`https://wall.alphacoders.com/api2.0/get.php?auth=1NpccB4VHkGQL4p72qMCZn2R8oevbMyhnA&method=search&count=1&term=${q}`) // Troque o API por uma API do site alpha coders
...
const ss = async(query) => { // Um print de site
    request({
        ...
        encoding: "binary",
        qs: {
            access_key: "b592aae51f4c4bd2958285ff7e6a4934", // Insira uma api do site apiflash url to image dentro das aspas
            url: query
        }
    }, (error, response, body) => {
        if (error) {
